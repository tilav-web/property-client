...
export interface GarageRentFormData {
  area: number | string;
  has_pit: boolean;
...
interface Props {
  data: GarageRentFormData;
  setData: (data: GarageRentFormData) => void;
  isSubmitting?: boolean;
}

const defaultFormData: GarageRentFormData = {
  area: "",
  has_pit: false,
...
export default function GarageRentForm({ data, setData, isSubmitting = false }: Props) {
  ...
  return (
    <Card className="mt-6">
      <CardHeader>
        <CardTitle>
          {t("property_form.garage_rent.title", { defaultValue: "Garaj ijarasi — tafsilotlar" })}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="max-w-xs">
          <Label htmlFor="garage_rent_area" className="flex items-center gap-1.5 text-xs mb-1.5">
            <Maximize size={14} />
