...
export default function BidPriceButton({
  ...
  // Agar foydalanuvchi e'lon egasi bo'lsa
  if (user?._id === property?.author?._id) {
    return (
      <button
        disabled
        className="bg-[#FF990063] flex items-center gap-2 px-3 py-2 rounded border border-black text-sm min-w-0 disabled:opacity-50"
      >
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <button
          onClick={handleClick}
          className="bg-[#FF990063] flex items-center gap-2 px-3 py-2 rounded border border-black text-sm min-w-0"
        >
          <img src={courtSvg} alt="Court svg" className="w-4 h-4" />
