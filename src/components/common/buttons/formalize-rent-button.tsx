...
export default function FormalizeRentButton({
  ...
  if (user?._id === property?.author?._id) {
    return (
      <button
        disabled
        className="bg-blue-500/30 flex items-center sm:gap-2 px-3 py-2 rounded border border-black text-sm min-w-0 disabled:opacity-50"
      >
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <button
          onClick={handleClick}
          className="bg-blue-500/30 flex items-center gap-2 px-3 py-2 rounded border border-black text-sm min-w-0"
        >
          <FileSignature className="w-4 h-4" />
